...
| **Framebuffer Architecture** | Canonical 256×192 logical canvas composed in Python via Pillow, exporting raw RGB24 row-major byte arrays (`w * h * 3` bytes). Decoupled from physical transport. Resolution-agnostic rendering. | [`docs/PROJECT.md`](../PROJECT.md) §2.2, §4.1, §4.2; [`docs/pm/02-requirements-matrix.md`](02-requirements-matrix.md) R004, R023, R024 |
| **Serial / Debug Interface Safety** | CH340 USB-UART debug connection strictly follows the **3-pin rule**: connect ONLY TX, RX, and common GND. NEVER connect 5 V or 3.3 V power from USB adapters to independently powered boards. | [`docs/pm/05-backlog.md`](05-backlog.md) JIRA Safety Rules; [`docs/EXPERIMENTS.md`](../EXPERIMENTS.md) EXP-013 |
| **HUB75 Signal Safety** | **NO HUB75 hot-plugging.** Always de-energize PSU and wait for discharge before connecting or disconnecting HUB75 ribbon cables. | [`docs/pm/05-backlog.md`](05-backlog.md) JIRA Safety Rules; [`docs/EXPERIMENTS.md`](../EXPERIMENTS.md) EXP-004, EXP-011 |
...
| **AF-043** | Huidu software Phase A EXP-007A: map UI (config, upload, live) | `CORRECT` | `03-wf4-single-panel.md` | `AF-043` | Retain EXP-007A vendor software workflow exploration (HD2018/HD2020: configuration, static file upload, live update interfaces). |
...
| **AF-053** | Gather ESP32 build materials: 2× SN74HCT245N, perfboard, header, caps | `CORRECT` | `04-esp32-single-panel.md` | `AF-053` | Retain pre-assembly materials staging (2× SN74HCT245N DIP-20, perfboard, 2×8 keyed HUB75 header, 2× 100nF ceramic caps, optional 1000µF bulk cap, wire). |
| **AF-054** | Stage 1 perfboard prep: mark rows, mount DIP-20 sockets/header | `CORRECT` | `04-esp32-single-panel.md` | `AF-054` | Retain as **Stage 1 (Perfboard Layout & Socket/Header Mounting)** of consolidated 5-stage build. Mount DIP-20 sockets/ICs and 2×8 keyed HUB75 header (notch facing outward). Verify alignment and pad isolation. |
...
| **AF-065** | Stage 12 Full adapter end-to-end continuity + no-shorts check | `CORRECT` | `04-esp32-single-panel.md` | `AF-065` | Retain as **Stage 5 (Full Adapter Unpowered Continuity & Isolation Audit)**. Perform 14 end-to-end signal continuity tests (ESP32 header pin → HUB75 pin), 4 IC power checks, and ≥20 cross-pin isolation tests (adjacent pins open, VCC↔GND open, signals↔GND open) before energizing. |
...
| **AF-067** | ESP32 + HCT + 1 panel physical test (EXP-011) | `CORRECT` | `04-esp32-single-panel.md` | `AF-067` | Retain EXP-011 single-panel validation (unpowered HUB75 connection, strict power-up order, standard test pattern suite, 10-point defect checklist, refresh rate/PSRAM logging, ≥1 hour stability). |
...
| **AF-173** | EXP-009 Step 1 coverage: WF2 stock firmware SPI flash backup | `CORRECT` | `05-wf2-investigation.md` | `AF-173` | Retain concrete flash backup procedure (`esptool.py read_flash` dual-dump hash match verify, CH340 3-pin rule enforcement, SHA256 logging). |
...
| `AF-028` | Create minimal Python 3 virtual environment | Provision Python 3 venv (`/home/sipeed/ai-frame-venv`), upgrade pip/setuptools/wheel, install C build headers (`libjpeg-dev`, `zlib1g-dev`, `libfreetype6-dev`), and generate `requirements-base.txt`. | `AF-027` | `software` `nano` |
...
| `AF-040` | Wire HD-WF4 DC power to PSU 5V rail | Install 18 AWG power leads from PSU 5V rail to WF4 power inputs; verify delivered harness construction; verify continuity (<1 Ω) and polarity under power-OFF conditions. | `AF-039`, `AF-024` | `hardware` `controller-wf4` `power` `safety-review` `polarity-verify` |
...
| `AF-043` | Characterize Huidu vendor software workflow (EXP-007 Phase A) | Test Huidu PC software (HD2018/HD2020); document configuration, static file upload, and live update UI surfaces; record transfer times. | `AF-042` | `software` `controller-wf4` `validation` |
...
| `AF-053` | Gather and inspect ESP32 adapter build materials | Stage 2× SN74HCT245N DIP-20 ICs, perfboard, 2×8 keyed HUB75 header, 2× 100nF ceramic caps, optional 1000µF bulk cap, and 24 AWG wire; verify quantities and pin pitch. | `AF-011`, `AF-052` | `hardware` `controller-esp32` `docs` |
| `AF-054` | Stage 1: Perfboard Layout & Socket/Header Mounting | Layout component positions on perfboard; mount and solder 2× DIP-20 sockets/ICs (notches aligned) and 2×8 keyed HUB75 header (notch facing outward); check pad isolation. | `AF-053` | `hardware` `controller-esp32` |
...
| `AF-065` | Stage 5: Full Adapter Unpowered Continuity & Isolation Audit | Perform complete unpowered electrical audit: verify 14 end-to-end signal continuity paths (ESP32 header pin → HUB75 pin), 4 IC power paths, and ≥20 cross-pin isolation tests (adjacent pins open, VCC↔GND open, signals↔GND open) before energizing. | `AF-061` | `hardware` `controller-esp32` `validation` `safety-review` |
...
| `AF-067` | Single panel physical display test via ESP32+HCT245 (EXP-011) | Connect panel #1 to adapter (HUB75 unpowered, strict power-up order); execute Standard Test Pattern Suite; evaluate 10-point defect checklist; record refresh rate and PSRAM metrics; run ≥1 hour. | `AF-066`, `AF-065`, `AF-021`, `AF-022` | `hardware` `controller-esp32` `power` `validation` |
...
| `AF-173` | Backup HD-WF2 stock firmware via SPI flash dump | Execute concrete dual-read SPI flash backup using `esptool.py read_flash` following CH340 3-pin rule; verify SHA256 hash match between read1 and read2; commit backup README. | `AF-076`, `AF-077` | `firmware` `controller-wf2` `docs` `safety-review` |
...
|---|---|---|---|
| `ADR-004` | HD-WF4 Primary Controller Candidate | Stock controller single-panel validation and programmatic control | `AF-038`, `AF-042`, `AF-045`, `AF-046` |
| `ADR-008` | LicheeRV Nano-W Compute Module | Host SBC bootstrap, memory audit (256 MB DDR3), and peripheral bringup | `AF-025`, `AF-026`, `AF-181`, `AF-182` |
| `ADR-009` | Python + Pillow Rendering Stack | Lightweight graphics composition and framebuffer pipeline | `AF-028`, `AF-029`, `AF-030`, `AF-031`, `AF-032` |
| `ADR-010` | MicroSD Card Storage | Host OS storage media flashing and boot verification | `AF-025` |
| `ADR-011` | Headless Linux OS Baseline | Headless operating system baseline on 256 MB RAM budget | `AF-025`, `AF-026`, `AF-182` |
| `ADR-012` | HD-WF2 Reference-Only Track | Reference spike characterization (never deployed in production V1) | `AF-076`, `AF-077`, `AF-172`, `AF-173`, `AF-078`, `AF-079` |
| `ADR-013` | Dual-Track Controller Architecture in M1 | Parallel evaluation of WF4 and ESP32 candidate controllers | `AF-042`, `AF-046`, `AF-067`, `AF-069`, `AF-080` |
| `ADR-014` | Deferred Architecture Selection | Feeds 13-criterion evaluation data from M1 experiments | `AF-047`, `AF-072`, `AF-075`, `AF-080` |
| `ADR-015` | SN74HCT245N Logic Level Buffering | 3.3V → 5V HUB75 signal level translation for ESP32 | `AF-053`, `AF-054`, `AF-055`, `AF-059`, `AF-061`, `AF-065`, `AF-067` |
| `ADR-016` | Architecture Winner Selection Matrix | Feeds single-panel metrics into 13-dimension decision matrix | `AF-047`, `AF-075`, `AF-080` |
| `ADR-017` | Nano-to-Controller Transport Protocol | Protocol evaluation across Huidu, UART, and native USB | `AF-033`, `AF-034`, `AF-044`, `AF-183`, `AF-184`, `AF-185`, `AF-186`, `AF-068`, `AF-070` |

---

*This refactor map serves as the authoritative specification for populating `docs/pm/backlog/00-hardware-receipt.md` and `docs/pm/backlog/01-power-bringup.md` during Pass 4A, and `docs/pm/backlog/02-nano-bootstrap.md` through `docs/pm/backlog/06-single-panel-gate.md` during Pass 4B.*
