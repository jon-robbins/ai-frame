...
### AF-094 — Draft ADR-016 controller architecture decision for review

**Milestone:** MG
**Depends on:** AF-093
**Labels:** decision controller-wf4 controller-esp32
**Context:** Prepare the ADR-016 draft from the completed EXP-014 matrix while preserving the current PENDING status until evidence and review are complete for the WF4 and three-controller ESP32 architecture (Nano + three ESP32 controllers).

#### Do
1. Summarize the WF4 and three-controller ESP32 evidence against the EXP-014 criteria and the project rule to prefer the simplest architecture that reliably satisfies V1.
2. Update the ADR-016 draft with the compared alternatives, rationale, consequences, rejected stacked-controller alternative, and direct evidence links.
3. Submit the draft for evidence/review acceptance; leave ADR-016 `PENDING` until that review records an accepted decision, and do not name a winner in this task.

#### Done when
- ADR-016 contains a reviewable, evidence-linked comparison and decision rationale for WF4 versus the three-controller ESP32 architecture (Nano + three ESP32 controllers).
- ADR-016 remains `PENDING` until the required evidence/review acceptance occurs; no controller winner is selected by this pre-decision task.

#### If it fails
Keep ADR-016 pending, identify the missing evidence or review objection, and return to AF-093 or the named candidate evidence source.

### AF-095 — Draft ADR-017 transport decision for review

**Milestone:** MG
**Depends on:** AF-094
**Labels:** decision validation
**Context:** Prepare the final Nano-to-controller transport decision from current transport evidence without committing an unreviewed protocol.

#### Do
1. Compare the current Huidu, USB, UART, native-USB, and local TCP/network transport evidence against reliable dynamic updates, reconnect behavior, maintainability, and V1 needs.
2. Update the ADR-017 draft with the alternatives, evidence links, rationale, and consequences; retain the project’s allowance for lower-rate dashboard updates where reliable.
3. Submit the draft for evidence/review acceptance and leave ADR-017 `PENDING` until that review records an accepted decision.

#### Done when
- ADR-017 contains an evidence-linked, reviewable transport comparison and rationale.
- ADR-017 remains `PENDING` until required evidence/review acceptance; no transport winner is asserted by this pre-decision task.

#### If it fails
Keep ADR-017 pending, record the missing transport evidence or review objection, and return to the relevant transport experiment or AF-094 context.
...
2. Read the reviewed ADR-016 result and apply the result to AF-098, AF-099, AF-100, AF-102, and AF-103; do not choose or revise ADR-016 or ADR-017 in this task.
...
5. Record the before/after task IDs, labels, resolved line, both accepted ADR statuses, and source decisions in the MG evidence record; only the selected branch is executable after the sweep.

#### Done when
- The sweep records both reviewed/accepted ADR statuses and applies the exact winner/loser field mutations to all five downstream branch entries before M3 can proceed.
