...
All tasks adhere to the compact schema specified in [`../runbooks/task-format.md`](../runbooks/task-format.md). The canonical definition of task granularity and compression rules lives there; the summary below is mirrored here for backlog readability.
...
| **M4: Six-Panel Full Prototype** | 🔒 Gated | 2×3 = 6 panels on winning controller architecture (256×192); arbitrary Nano-driven content anywhere on canvas; all seams correctly crossed; acceptable refresh rate; EXP-015 4-brightness thermal pass; EXP-016 ×5 boot, controller-transport, and Wi-Fi recovery pass. |
| **MA: Application Software** | Review Gate | All 12 JIRA software stages complete; production dashboard runtime with widgets (time/date, weather, Google Calendar, Spotify now-playing/artwork, decorative graphics/icons, and other API-driven dashboards), API-failure caching/stale-data behavior, live-data resume, and offline-fallback supervision operational on Nano. |
| **MR: Reliability & Burn-In** | Review Gate | Starts after MA: display brightness ceiling calibrated and logged, then a 24-hour continuous unattended dashboard-normal stability test with finished MA runtime, without crash, thermal runaway, or visual artifacting. |
| **Cond-X: Custom ESP32 PCB** | Conditional Gate | Skip if ADR-016 selects WF4. If active: 19 KiCad steps (schematic ERC, layout DRC, Gerber/BOM/CPL generation, fabrication, continuity testing, row validation, swap out perfboard for PCB). |
...
| [`11-reliability.md`](11-reliability.md) | MR | Brightness ceiling calibration, then 24-hour continuous burn-in with the finished MA dashboard runtime |
